...
interface DiscountFlyerCardProps {
  onSave: (data: DiscountFlyerData) => Promise<void>;
  onDelete?: () => void;
  initialData?: Partial<DiscountFlyerData>;
  isNew?: boolean;
}

export interface DiscountFlyerData {
  id?: string;
  title: string;
  description?: string;
  discountDate: Date;
...
export default function DiscountFlyerCard({ 
  onSave, 
  onDelete, 
  initialData, 
  isNew = true 
}: DiscountFlyerCardProps) {
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>{isNew ? '새 할인 전단지' : '할인 전단지 수정'}</span>
          {!isNew && onDelete && (
            <Button
              variant="destructive"
              size="sm"
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">할인 날짜</label>
          <Popover>
        ...
        <div className="space-y-2">
          ...
          <Textarea
            placeholder="할인 전단지에 대한 추가 설명을 입력하세요"
            value={description}
        ...
        {/* 이미지 업로드 */}
        <div className="space-y-2">
          <label className="text-sm font-medium">전단지 이미지</label>
        ...
        {products.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium">인식된 상품 목록</label>
              <Button
                variant="outline"
                size="sm"
